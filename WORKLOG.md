...
| Antigravity (Pair Programming) | Implement curriculum metadata & complete Math 6 DAG | ✅ Done | Thực hiện SQL migration thêm các cột curriculum_chapter, curriculum_order, concept_depth; viết script `populate_curriculum_metadata.py` để phân loại concept_type và tính độ sâu; nâng cấp seed_math_relations lên 110 edges kết nối 111/113 active concepts (98%). | 2h |
| Antigravity (Pair Programming) | Fix backend CI errors & optimize test_runner.py | ✅ Done | Sửa đổi mã lỗi `status_code=500` sang `503` trong `exam_routes.py` tuân thủ backend error contracts; nâng cấp cấu hình backend checks trong `supabase_config.py` trả về `RuntimeError` khi key không hợp lệ; loại bỏ ruff lint errors trong các file script và sửa lỗi `test_runner.py` bị sập I/O trên Windows bằng cách chuyển sang chạy `uv run pytest tests/` giới hạn thư mục test. | 1.5h |

**Tổng kết ngày:** Nguyễn Phương Nam hoàn thiện chất lượng và đồng bộ 100% các concept Toán học trên toàn bộ hệ thống; loại bỏ triệt để các tệp mock thừa và các thuật ngữ máy học để đảm bảo uy tín demo sư phạm; cấu hình git tracking dữ liệu câu hỏi giải quyết lỗi CI/CD từ xa; hoàn thành bộ kiểm thử tự động đạt 331 tests (100% pass trên local và CI). Nguyễn Vũ Trọng thiết lập hệ thống Quản lý Đề thi thích ứng trên Supabase và FastAPI với RLS và kiểm thử đầy đủ, đồng thời cập nhật tài liệu SSoT `PROJECT-CONTEXT.md`. Đồng thời, gộp nhóm thành công 60 concepts trùng lặp/kỹ năng con, mở rộng schema thêm curriculum chapters/order/depth, seed bổ sung 30 quan hệ để kết nối 98% toàn bộ bản đồ tri thức Toán 6. Cuối cùng, sửa chữa triệt để các lỗi kiểm thử CI backend và ruff linting trong các script, tối ưu hóa `test_runner.py` chạy ổn định trên môi trường Windows local và Git.
