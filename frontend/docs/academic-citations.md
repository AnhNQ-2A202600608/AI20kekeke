...
*   **Production Code:** [bkt.py](file:///d:/code/AI20kekeke/src/services/adaptive/bkt.py)
...
*   **Evaluation Suite:** Verified in [exp2_bkt_validation.py](file:///d:/code/AI20kekeke/eval/exp2_bkt_validation.py) using pure BKT simulated emission and BKT state prediction accuracy (calculating Wilcoxon-Mann-Whitney ROC AUC).
...
### Implementation in Mentora
*   **Production Code:** [elo.py](file:///d:/code/AI20kekeke/src/services/adaptive/elo.py)
...
*   **Evaluation Suite:** Verified in [exp1_elo_convergence.py](file:///d:/code/AI20kekeke/eval/exp1_elo_convergence.py) using seeded student populations to measure convergence speed (RMSE) of estimated student ability against the latent truth.
...
### Implementation in Mentora
*   **Production Code:** [bandit.py](file:///d:/code/AI20kekeke/src/services/adaptive/bandit.py)
...
*   **Evaluation Suite:** Verified in [exp3_bandit_comparison.py](file:///d:/code/AI20kekeke/eval/exp3_bandit_comparison.py) comparing LinUCB against Random and Greedy baselines under paired Common Random Numbers (CRN), validating reward curves and Zone of Proximal Development (ZPD) targeting hit rates.
...
### Implementation in Mentora
*   **Production Code:** [graph_propagation.py](file:///d:/code/AI20kekeke/src/services/adaptive/graph_propagation.py)
...
*   **Evaluation Suite:** Verified in [exp4_graph_propagation.py](file:///d:/code/AI20kekeke/eval/exp4_graph_propagation.py) asserting exact computed decay bounds on cyclic/stress graphs and verifying write-through cache invalidation.
...
### Implementation in Mentora
*   **Production Code:** [forgetting.py](file:///d:/code/AI20kekeke/src/services/adaptive/forgetting.py)
*   **Mathematics:** Models memory decay based on stability days:
    ...
    $$S_{\text{new}} = \begin{cases} 
      ...
      S & \text{otherwise}
    \end{cases}$$
*   **Evaluation Suite:** Verified in [exp5_forgetting_decay.py](file:///d:/code/AI20kekeke/eval/exp5_forgetting_decay.py) validating FSRS lazy decay calculations, stability updates, and daily longitudinal retention simulation.
...
### Implementation in Mentora
*   **Production Code:** Graph relationships loaded into the database schema and traversed via `propagate_mastery` inside [graph_propagation.py](file:///d:/code/AI20kekeke/src/services/adaptive/graph_propagation.py) are grounded in Graphusion's hierarchical concept modeling (using the LectureBankCD dataset) and Socratic learning pathways.
*   **Evaluation Suite:** Evaluated in [exp4_graph_propagation.py](file:///d:/code/AI20kekeke/eval/exp4_graph_propagation.py) which tests directed cyclic and acyclic graph traversals, representing the global-fusion sub-graph structures designed in the Graphusion framework.
