...
*   **Evaluation Suite:** Được xác thực trong [exp2_bkt_validation.py](file:///d:/code/AI20kekeke/eval/exp2_bkt_validation.py) bằng mô phỏng phản hồi thuần túy BKT và tính toán độ chính xác dự đoán trạng thái (ROC AUC).
...
*   **Evaluation Suite:** Được xác thực trong [exp1_elo_convergence.py](file:///d:/code/AI20kekeke/eval/exp1_elo_convergence.py) bằng quần thể học sinh mô phỏng để đo tốc độ hội tụ (RMSE) của năng lực ước lượng so với năng lực ẩn thực tế.
...
*   **Evaluation Suite:** Được xác thực trong [exp3_bandit_comparison.py](file:///d:/code/AI20kekeke/eval/exp3_bandit_comparison.py) so sánh LinUCB với các baseline ngẫu nhiên và tham lam dưới phương pháp Số ngẫu nhiên chung (CRN), kiểm chứng đường cong phần thưởng và tỷ lệ trúng mục tiêu ZPD.
...
*   **Evaluation Suite:** Được xác thực trong [exp4_graph_propagation.py](file:///d:/code/AI20kekeke/eval/exp4_graph_propagation.py) kiểm thử các suy giảm trên đồ thị cyclic/stress và kiểm chứng cơ chế invalidation của write-through cache.
...
*   **Mathematics:** Mô hình hóa suy giảm trí nhớ dựa trên số ngày ổn định:
    ...
    $$S_{\text{new}} = \begin{cases} 
      ...
      S & \text{otherwise}
    \end{cases}$$
*   **Evaluation Suite:** Được xác thực trong [exp5_forgetting_decay.py](file:///d:/code/AI20kekeke/eval/exp5_forgetting_decay.py) kiểm chứng tính toán lazy decay của FSRS, cập nhật độ ổn định và mô phỏng duy trì trí nhớ dài hạn theo ngày.
...
### Implementation in Mentora
*   **Production Code:** Các mối quan hệ khái niệm được nạp vào schema cơ sở dữ liệu và duyệt qua hàm `propagate_mastery` trong [graph_propagation.py](file:///d:/code/AI20kekeke/src/services/adaptive/graph_propagation.py) được xây dựng dựa trên mô hình hóa khái niệm phân tầng (sử dụng tập dữ liệu LectureBankCD) và lộ trình học tập Socratic của Graphusion.
*   **Evaluation Suite:** Được đánh giá trong [exp4_graph_propagation.py](file:///d:/code/AI20kekeke/eval/exp4_graph_propagation.py) qua các kịch bản kiểm thử đồ thị có hướng tuần hoàn và không tuần hoàn, mô phỏng các cấu trúc đồ thị con được thiết kế trong khung Graphusion.
