...
RECOMMENDATIONS_HEURISTICS = {
    "agent": {
        "keywords": ["agent", "multi-turn", "chatbot", "conversation", "tool calling", "langgraph", "reasoning"],
        "reason": "Dialogue flow or tool-calling reasoning detected. Recommend AI Agent orchestration.",
        "dependency": "langgraph, langchain_core"
    },
    "rag": {
        "keywords": ["rag", "semantic search", "vector database", "chromadb", "document qa", "retrieval"],
        "reason": "Text document lookup or retrieval-augmented generation detected. Recommend RAG System.",
        "dependency": "chromadb"
    },
    "computer_vision": {
        "keywords": ["image", "object detection", "contour", "opencv", "computer vision", "video", "frame"],
        "reason": "Image processing or visual target analysis detected. Recommend Computer Vision module.",
        "dependency": "cv2"
    },
    "prediction": {
        "keywords": ["predict", "forecast", "classification", "regressor", "scikit-learn", "numpy", "tabular"],
        "reason": "Tabular forecast or numerical machine learning modeling detected. Recommend Prediction Engine.",
        "dependency": "numpy, sklearn"
    },
    "optimization": {
        "keywords": ["optimization", "scipy", "linear programming", "operations research", "minimize", "maximize"],
        "reason": "Operational constraints or resource allocation variables detected. Recommend Optimization Engine.",
        "dependency": "scipy"
    },
    "analytics": {
        "keywords": ["analytics", "dataframe", "pandas", "data profile", "correlation", "aggregation"],
        "reason": "Exploratory data analysis or column aggregations detected. Recommend Data Analytics module.",
        "dependency": "pandas"
    }
}
...
def main() -> None:
    ...
    parser.add_argument("--output", help="Optional override of challenge workspace destination folder")
    ...
    if isinstance(rubrics, dict):
        ...
        total_weight = sum(float(w) for w in rubrics.values() if str(w).replace('.', '', 1).isdigit())
    ...
    if total_weight != 100.0:
        logger_warn(f"Total rubric weight equals {total_weight}%, which is atypical (expected 100%).")
    ...
    workspace_dir = Path(args.output) if args.output else Path(__file__).resolve().parents[1] / "challenges" / slug
    ...
    (workspace_dir / "challenge.yaml").write_text(json.dumps(challenge_config, indent=2), encoding="utf-8")
    ...
    (workspace_dir / "architecture.md").write_text("# Proposed Architecture Design\n", encoding="utf-8")
    ...
    (workspace_dir / "modules_config.json").write_text(json.dumps(modules_config, indent=2), encoding="utf-8")

    print(f"\nSuccess: Scaffolded independent workspace for '{args.challenge_name}' at:\n  {workspace_dir}\n")
