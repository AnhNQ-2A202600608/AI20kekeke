...
from src.core.module import ModuleRegistry, ModuleManifest
...
def temp_modules_dir():
    ...
    mod1_dir.mkdir()
    (mod1_dir / "module.json").write_text(json.dumps({
        "id": "mod_valid",
        "name": "Valid Module",
        "version": "0.1.0",
        "category": "test",
    ...
    mod2_dir.mkdir()
    (mod2_dir / "module.json").write_text(json.dumps({
        "id": "mod_missing_dep",
    ...
    mod3_dir.mkdir()
    (mod3_dir / "module.json").write_text(json.dumps({
        "id": "mod_missing_env",
