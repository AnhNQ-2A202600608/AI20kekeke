...
from src.core.errors import AppError, ProviderError
...
class HttpIntegrationClient:
    ...
    def __init__(self, base_url: str = "", timeout_seconds: float = 10.0, max_retries: int = 3) -> None:
    ...
    async def request(
        ...
        full_url = f"{self.base_url.rstrip('/')}/{url_path.lstrip('/')}" if self.base_url else url_path
        ...
        while True:
            ...
            try:
                logger.info(
                    ...
                    attempt, self.max_retries, method, full_url, redacted_headers
                )
                ...
                logger.error(
                    "HTTP Error response (%d): %s | Body: %s",
                    status_code, exc, exc.response.text
                )
                ...
                if status_code in (400, 401, 403, 404, 422) or attempt >= self.max_retries:
                    raise ProviderError(
                        provider="HTTPClient",
                        reason=f"Status {status_code}: {exc.response.text}"
                    ) from exc
                ...
                if attempt >= self.max_retries:
                    raise ProviderError(
                        provider="HTTPClient",
                        reason=f"Connection/Timeout failure: {exc}"
                    ) from exc
