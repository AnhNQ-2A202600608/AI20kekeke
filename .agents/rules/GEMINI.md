...
### 📝 Nhật ký tương tác AI (AI Logging Rule)

**MANDATORY:** Sau mỗi lượt tương tác (khi nhận prompt và phản hồi người dùng), AI bắt buộc phải mở file `@[.ai-log/session.jsonl]` và chèn thêm một dòng log dạng JSON hợp lệ ghi lại prompt của người dùng cùng tóm tắt câu trả lời (xem chi tiết định dạng tại `@[.agents/rules/ai-log-hook.md]`).

### 🚫 Cấm sử dụng Trình duyệt (Browser Subagent Ban)

**TUYỆT ĐỐI KHÔNG** tự ý sử dụng `browser_subagent` để mở browser hoặc chạy web automation test. Việc này gây tiêu tốn rất nhiều quota và tài nguyên của người dùng. Nếu cần kiểm thử, hãy sử dụng các hình thức kiểm thử tĩnh, API call, Unit test nội bộ, hoặc yêu cầu người dùng tự kiểm tra thủ công.
