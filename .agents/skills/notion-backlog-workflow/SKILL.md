...
## Pull Request (PR) Integration & Automation

When the user requests the AI to create a Pull Request (PR) or completes a task:
1. **Auto-Update Trigger**: The AI must automatically check if there is an associated backlog item on Notion.
2. **Retrieve details**: Query or search for the task using the task ID (`Mã việc`) or task title.
3. **Handle Ambiguity / Missing Info**:
   - If the AI cannot find the corresponding task, or is unsure which task to update, it **must ask the user** to clarify (e.g., "Tôi thấy bạn muốn tạo PR, task này tương ứng với Mã việc nào trên Notion?").
   - If the AI is missing information required for the `Completion Report` (such as verification results, changed files summary, or status details), it **must ask the user** for these details before performing the update.
4. **Action**: Append the `Completion Report` to the page body and update the task's `Trạng thái` to `Hoàn thành` (or keep as `Đang làm` if partial/blocked) using the Notion API/MCP tool.
## Security
