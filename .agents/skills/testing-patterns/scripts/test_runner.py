...
# Fix Windows console encoding commented out to prevent pytest I/O closed file error
# try:
...
def run_tests(cmd: list, cwd: Path) -> dict:
    ...
    try:
        ...
        # Prepend virtualenv Scripts and bin to PATH for absolute isolation on Windows
        venv_scripts = str(cwd / ".venv" / "Scripts")
        venv_bin = str(cwd / ".venv" / "bin")
        env["PATH"] = venv_scripts + os.pathsep + venv_bin + os.pathsep + env.get("PATH", "")
        ...
        proc = subprocess.run(
            ...
            encoding='utf-8',
            errors='replace',
            timeout=120,  # 2 min timeout for tests
            env=env
        )
