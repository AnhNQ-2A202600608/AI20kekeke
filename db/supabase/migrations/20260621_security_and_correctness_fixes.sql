...
-- 1. Định nghĩa lại hàm submit_attempt_v3 sửa lỗi ép kiểu UUID, stability_days cap, và đồng bộ hóa thuật toán (loại bỏ partial BKT, loại bỏ graph propagation đồng bộ)
...
BEGIN
    -- -------------------------------------------------------------------------
    -- 1. Idempotency Check: Sửa lỗi ép kiểu UUID (qa.id thay vì to_jsonb(qa))
    -- -------------------------------------------------------------------------
    IF EXISTS (
    ...
    ELSE
        ...
        IF p_response_time_ms IS NOT NULL AND p_actual_score >= 0.75 THEN
            ...
            BEGIN
                -- Bảo vệ: response_time_ms nằm trong khoảng [300, 3600000]ms
                v_clamped_time := greatest(300.0, least(3600000.0, p_response_time_ms::numeric));
                v_avg_time := coalesce(v_question.avg_response_time_ms, 30000.0);
                IF v_avg_time > 0 THEN
                    v_speed_ratio  := v_clamped_time / v_avg_time;
    ...
    -- -------------------------------------------------------------------------
    -- 6. Thực hiện update Elo, BKT và counters (Atomic + Cap stability_days tối đa 36500)
    -- -------------------------------------------------------------------------
    ...
    -- -------------------------------------------------------------------------
    -- 9. Trả về kết quả (Không lan truyền đồ thị đồng bộ nữa, chuyển sang async Python ở API)
    -- -------------------------------------------------------------------------
    RETURN jsonb_build_object(
        'new_student_elo',      v_new_student_elo,
        'new_question_elo',     v_new_question_elo,
        ...
        'weakness_flag',         v_weakness_flag,
        'propagated_concepts',   jsonb_build_array(), -- Mảng rỗng
        'stability_days',        v_new_stability
    );
END;
$$;
